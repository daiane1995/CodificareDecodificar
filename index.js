...
codeDecode.forEach((radio) => {
    radio.addEventListener("change", function (e) {
        ...
        if (e.target.value == "decode") {
            btn.innerHTML = "Decode message!"
        } else {
            btn.innerHTML = "Encode message!"
        }
    });
});
...
function codeCesar(txt, inc) {
    ...
    for (var i = 0; i < txt.length; i++) {
        if (txt[i].charCodeAt() >= 65 && txt[i].charCodeAt() <= 90) {
            Cesinha = ((txt[i].charCodeAt() - 65 + inc) % 26) + 65;
            resultado.push(String.fromCharCode(Cesinha));
        } else if (txt[i].charCodeAt() >= 97 && txt[i].charCodeAt() <= 122) {
          Cesinha = ((txt[i].charCodeAt() - 97 + inc) % 26) + 97;
            resultado.push(String.fromCharCode(Cesinha));
        } else {
...
function decodeCesar(txt, inc) {
    ...
    for (var i = 0; i < txt.length; i++) {
        if (txt[i].charCodeAt() >= 65 && txt.charCodeAt() <= 90) {
            Cesinha = ((txt[i].charCodeAt() - 90 - inc) % 26) + 90
            resultado.push(String.fromCharCode(Cesinha))
        } else if (txt[i].charCodeAt() >= 97 && txt[i].charCodeAt() <= 122) {
            Cesinha = ((txt[i].charCodeAt() - 122 - inc) % 26) + 122
            resultado.push(String.fromCharCode(Cesinha))
        } else {
            resultado.push(txt[i])
        }
